        ...
        let succeeded = match ipc::duplicate(SERVICE_CONTROL_OBSERVER_HANDLE, Rights::SEND) {
            Ok(observer) => {
                let result = match service {
                    ...
                    None => sv::list(observer),
                };
                let close_result = ipc::close(observer);
                result.is_ok() && close_result.is_ok()
            }
            Err(_) => false,
        };
        if succeeded {
            self.output(if service.is_some() {
                SV_STATUS_COMPLETE
            } else {
                SV_LIST_COMPLETE
            });
...
fn detect_builtin(command: &[u8]) -> Option<Builtin> {
    ...
    match &command[..token_end] {
        ...
        b"sv" => Some(detect_service_builtin(arguments)),
        _ => None,
    }
}

fn detect_service_builtin(arguments: &[u8]) -> Builtin {
    let arguments = trim_horizontal(arguments);
    if arguments == b"list" {
        return Builtin::ServiceList;
    }
