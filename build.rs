...
fn main() {
    ...
    let nullfs_block_device_loss_test_bios_image =
        output_directory.join("nullstar-os-nullfs-block-device-loss-test-bios.img");
    build_image(NULLFS_BLOCK_DEVICE_LOSS_TEST_BOOT_MODE)
        .create_bios_image(&nullfs_block_device_loss_test_bios_image)
    ...
    println!(
        "cargo:rustc-env=NULLFS_BLOCK_DEVICE_LOSS_TEST_BIOS_IMAGE={}",
        nullfs_block_device_loss_test_bios_image.display()
    );
    println!(
