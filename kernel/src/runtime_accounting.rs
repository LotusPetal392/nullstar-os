...
impl RuntimeAccounting {
    ...
    ) -> Result<ProcessRuntime, AccountingError> {
        let index = self.process_index(process_id)?;
        if self.threads.iter().any(|t| t.process_id == process_id) {
    ...
    pub fn register_thread(
        ...
        if self.threads.iter().any(|t| t.thread_id == thread_id) {
    ...
    pub fn unregister_thread(
        &mut self,
        thread_id: ThreadId,
    ) -> Result<ThreadRuntime, AccountingError> {
    ...
    pub fn register_cpu(&mut self, cpu: CpuId) -> Result<(), AccountingError> {
        if self.cpus.iter().any(|c| c.cpu == cpu) {
    ...
    pub fn account_tick(
        ...
        match mode {
            CpuMode::User => {
                self.threads[ti].user_ticks = self.threads[ti].user_ticks.saturating_add(1);
                self.processes[pi].user_ticks = self.processes[pi].user_ticks.saturating_add(1);
            }
            CpuMode::Kernel => {
                self.threads[ti].kernel_ticks = self.threads[ti].kernel_ticks.saturating_add(1);
                self.processes[pi].kernel_ticks = self.processes[pi].kernel_ticks.saturating_add(1);
            }
        }
        Ok(())
    }
    ...
    pub fn process(&self, id: ProcessId) -> Result<ProcessRuntime, AccountingError> {
        Ok(self.processes[self.process_index(id)?])
    }

    pub fn thread(&self, id: ThreadId) -> Result<ThreadRuntime, AccountingError> {
        Ok(self.threads[self.thread_index(id)?])
    }

    pub fn cpu(&self, id: CpuId) -> Result<CpuRuntime, AccountingError> {
        Ok(self.cpus[self.cpu_index(id)?])
    }
    ...
    fn process_index(&self, id: ProcessId) -> Result<usize, AccountingError> {
        self.processes
            .iter()
            .position(|p| p.process_id == id)
            .ok_or(AccountingError::InvalidProcess)
    }

    fn process_mut(&mut self, id: ProcessId) -> Result<&mut ProcessRuntime, AccountingError> {
        let i = self.process_index(id)?;
        Ok(&mut self.processes[i])
    }

    fn thread_index(&self, id: ThreadId) -> Result<usize, AccountingError> {
        self.threads
            .iter()
            .position(|t| t.thread_id == id)
            .ok_or(AccountingError::InvalidThread)
    }

    fn cpu_index(&self, id: CpuId) -> Result<usize, AccountingError> {
        self.cpus
            .iter()
            .position(|c| c.cpu == id)
...
mod tests {
    ...
    fn pid(v: u64) -> ProcessId {
        ProcessId::from_raw(v).unwrap()
    }

    fn tid(v: u64) -> ThreadId {
        ThreadId::from_raw(v).unwrap()
    }

    fn cpu(v: usize) -> CpuId {
        CpuId::from_raw(v).unwrap()
    }

    #[test]
    fn aggregates_thread_process_and_cpu_runtime() {
        let mut a = RuntimeAccounting::new();
        a.register_process(pid(1)).unwrap();
        a.register_thread(tid(1), pid(1)).unwrap();
        a.register_cpu(cpu(0)).unwrap();
        a.account_tick(tid(1), cpu(0), CpuMode::User).unwrap();
        a.account_tick(tid(1), cpu(0), CpuMode::Kernel).unwrap();
        a.account_context_switch(cpu(0), tid(1), true).unwrap();
        assert_eq!(a.thread(tid(1)).unwrap().user_ticks, 1);
        assert_eq!(a.thread(tid(1)).unwrap().kernel_ticks, 1);
        assert_eq!(a.process(pid(1)).unwrap().cpu_ticks, 2);
        assert_eq!(a.cpu(cpu(0)).unwrap().busy_ticks, 2);
    ...
    fn accounts_idle_time_and_migration() {
        ...
        a.register_thread(tid(1), pid(1)).unwrap();
        a.register_cpu(cpu(0)).unwrap();
        a.register_cpu(cpu(1)).unwrap();
        a.account_idle_tick(cpu(0)).unwrap();
        a.account_migration(tid(1), cpu(0), cpu(1)).unwrap();
        assert_eq!(a.cpu(cpu(0)).unwrap().idle_ticks, 1);
        assert_eq!(a.cpu(cpu(0)).unwrap().migrations_out, 1);
        assert_eq!(a.cpu(cpu(1)).unwrap().migrations_in, 1);
    }
    ...
    fn tracks_memory_and_peak_without_underflow() {
        let mut a = RuntimeAccounting::new();
        a.register_process(pid(1)).unwrap();
        a.charge_memory(pid(1), 4096).unwrap();
        a.charge_memory(pid(1), 4096).unwrap();
        a.release_memory(pid(1), 2048).unwrap();
        a.charge_mapped_memory(pid(1), 4096).unwrap();
        a.release_mapped_memory(pid(1), 1024).unwrap();
        let p = a.process(pid(1)).unwrap();
        assert_eq!(p.memory_bytes, 6144);
        assert_eq!(p.peak_memory_bytes, 8192);
        assert_eq!(p.mapped_bytes, 3072);
        assert_eq!(
            a.release_memory(pid(1), 8192),
    ...
    fn process_reclamation_waits_for_threads() {
        ...
        a.register_thread(tid(1), pid(1)).unwrap();
        assert_eq!(
            a.unregister_process(pid(1)),
            Err(AccountingError::InvalidThread)
        );
        a.unregister_thread(tid(1)).unwrap();
