...
PID 1 treats the path as clean only after both exact `CLEAN_UNMOUNTED` and final exit `0`, in either order. It then closes the old endpoint, creates a fresh endpoint object, and starts and registers a strictly newer generation before completing the fence. A timeout, malformed or mismatched event, attached event capability, lifecycle `FAILED`, or early/nonzero exit is not durability proof; PID 1 offlines the exact generation, forces KILL/reap when needed, and replacement mounts through dirty recovery. Controlled restart still uses no failure backoff or restart budget. `NFLC` is private supervision traffic: it changes neither `NSVC` v1 nor the public filesystem version 1 `Request`, `Reply`, or operations.
