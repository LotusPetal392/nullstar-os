...
Filesystem restart also keeps the restart fence pending through replacement startup. Its enforced order is final child status, ABI 1.13 offlining of the exact old provider generation and failure/wakeup of that generation's work, closure of the old endpoint handle, creation of a fresh endpoint object, startup and registration under a strictly newer generation, and only then fence completion. Writable NullFS is not withdrawn before final exit because pre-exit quiesce and `SYNC` remain future work.
...
Those remain later lifecycle and service-management work. The present native deliverable is capability-separated observation and mutation over ordinary endpoints, backed by the allocation-free fixed-record contract and its host-testable canonical encoding, decoding, validation, correlation, and desired-state transition rules. Logging provides the first live nonblocking start/stop convergence pilot. Filesystem restart now uses exact-generation provider offlining, deterministic failure of pending proxy requests, and fresh generation endpoints. Filesystem `Start` and `Stop` remain exactly `Unsupported`, and writable NullFS still requires an orderly pre-exit quiesce/`SYNC` boundary before live stop can safely withdraw it.
