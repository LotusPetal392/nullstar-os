...
A successful `Restart` response means PID 1 committed restart intent and accepted responsibility for replacing the service; it does not mean the replacement is ready. The response reports the old generation as `Terminating` with desired state `Running`. Controlled replacement uses zero failure backoff, does not consume the automatic failure-restart budget, and assigns the next manager-owned generation to the replacement. Logging keeps restart intent pending across bounded replacement startup until a later bounded mutation pass observes the ingress empty, so queued duplicates receive `Busy`. PID 1 first requests cooperative logging termination, then escalates after a bounded grace period to uncatchable, unblockable signal 9 so an ignored or masked termination request cannot stall convergence indefinitely.

Filesystem restart also keeps the restart fence pending through replacement startup. Controlled NullFS restart privately queues an exact 24-byte `NFLC` version 1 `QUIESCE` frame on the existing FIFO filesystem request endpoint, behind earlier work. The service finishes earlier requests, enters a quiesced state that processes no later public operations, and emits `QUIESCED` with the same nonzero generation and transition ID. PID 1 then offlines that exact generation, waking tail work with `EIO`, and queues `UNMOUNT`. The service closes all core open handles, calls `try_unmount` to sync and publish a clean superblock, emits exact `CLEAN_UNMOUNTED`, and exits `0`.

PID 1 treats the path as clean only after both exact `CLEAN_UNMOUNTED` and final exit `0`, in either order. It then drains the old generation job, terminating any escaped descendants, closes the old endpoint, creates a fresh endpoint object, and starts and registers a strictly newer generation before completing the fence. A timeout, malformed or mismatched event, attached event capability, lifecycle `FAILED`, or early/nonzero exit is not durability proof; PID 1 offlines the exact generation, terminates and drains the whole generation job, and replacement mounts through dirty recovery. Controlled restart still uses no failure backoff or restart budget. `NFLC` is private supervision traffic: it changes neither `NSVC` v1 nor the public filesystem version 1 `Request`, `Reply`, or operations.

A successful logging `Stop` response is sent after desired state becomes `Stopped` and process-group termination is accepted, but before final exit. PID 1 withdraws producer and observer routes before servicing later route resolutions. Existing delegated provider handles are not revoked, and the old generation's source endpoints remain owned until final child status and complete job drainage; they are then closed and never reused. If cooperative termination does not produce final child status within the bounded grace period, PID 1 sends signal 9 to the complete flat job. It consumes the direct leader status independently for policy and waits for job `ECHILD` before replacement. A successful logging `Start` response commits desired state `Running` before spawn or readiness. PID 1 launches at most one generation, accepts readiness only for the current starting child, and publishes fresh route objects under the next manager-owned generation.
...
Each starting logging generation has a bounded readiness budget. Expiry forces the complete job to exit with signal 9; direct-leader final-status handling then charges the bounded restart/backoff policy or moves the service to `Failed`, and full job drainage prevents escaped descendants from overlapping the next attempt.
...
- a general service launch, live filesystem stop, readiness, health, or restart-policy framework beyond PID 1's existing hard-coded supervision;
...
- changes to `NSVC` v1, endpoint forms, or attachment primitives. The existing signal ABI supplies uncatchable, unblockable signal 9 for bounded supervisor escalation, while ABI 1.13 separately adds the narrow PID-1-only filesystem-provider offlining syscall.

Those remain later lifecycle and service-management work. The present native deliverable is capability-separated observation and mutation over ordinary endpoints, backed by the allocation-free fixed-record contract and its host-testable canonical encoding, decoding, validation, correlation, and desired-state transition rules. Logging provides the first live nonblocking start/stop convergence pilot. Controlled NullFS restart now adds private quiesce, proven clean unmount, exact-generation offlining, deterministic failure of tail proxy requests, dirty-recovery fallback, and fresh-generation endpoints without changing `NSVC` v1. Filesystem `Start` and `Stop` remain exactly `Unsupported`; this restart-only boundary must not be described as live filesystem stop/start.
