...
1. **Portal mediation and grant-backed authority** — add rooted live-I/O forwarding and
   stored-identity restoration behind the implemented failure-atomic, rights-reduced resource
   endpoints, then implement compositor and portal transports around the gesture admission, live
   resource resolver, and permission-store records.
